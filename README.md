...
### 第 1 段階: 実測データから材料を準備する（`load_group_spectra`）
...
### 第 2 段階: ピクセルごとにスペクトルを合成する（`generate_alis_mock_cube`）
...
```
src/
  alis_mock_generator.py      # コア: データ読み込み・スペクトル合成（matplotlib 非依存）
  alis_mock_plot.py           # 可視化: plot_* 関数群
...
cube, wavelengths, ice_map, bd_map, metadata, group_data = generate_alis_mock_cube(config)
