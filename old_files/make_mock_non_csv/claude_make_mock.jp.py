...
def predict_gradient(grain_size_um: float, reflectance_1500: float) -> float:
    ...
    """
    # Eq. (1) の計算: gradient = 0.0517 * grain_size_um + 26.0 * reflectance_1500 - 17.4
    # Araki & Saiki (2025) の線形回帰式
...
def dry_mineral_spectrum(
    ...
    # Table 2 の fine (75–125 μm) と coarse (180–250 μm) の値を基に内挿
    # grain_size_um が 100 μm のとき t=0 (fine), 215 μm のとき t=1 (coarse)
    t = (grain_size_um - 100.0) / (215.0 - 100.0)  # 内挿パラメータ t の計算 (0-1 の範囲)
    t = np.clip(t, 0.0, 1.0)  # t を 0-1 にクリップ (範囲外の場合は端の値を使用)
    # 線形内挿: ref_1500 = (1-t) * fine + t * coarse
    ...
    # → 短波長側が低い傾斜を持つ
    # 簡易モデル: 750 nm で 0.85*ref_1500, 1500 nm で ref_1500 に達する線形
    continuum = ref_1500 * (0.85 + 0.15 * (wavelengths - 750) / (1500 - 750))
    continuum = np.clip(continuum, 0.0, None)  # 負の値を防ぐ

    # 鉱物固有の 1 μm 付近吸収帯（ガウス型）
    # 中心波長 mineral.abs_1um_center, 幅 mineral.abs_1um_width, 深さ mineral.abs_1um_depth
...
def compute_band_depth(
    ...
    # 中心でのコンティニュアム値 Rc の計算
    frac = (wavelengths[idx_center] - wavelengths[idx_left]) / \
           (wavelengths[idx_right] - wavelengths[idx_left])  # 左肩からの割合
    rc = r_left + frac * (r_right - r_left)  # 線形内挿
    rb = spectrum[idx_center]  # バンド中心の反射率
...
def create_ice_map(config: ALISMockConfig, rng: np.random.Generator) -> np.ndarray:
    ...
    if config.ice_pattern == "uniform":
        ...
        # 中心 (0.5, 0.5) からの距離 r を計算
        r = np.sqrt((xx - 0.5) ** 2 + (yy - 0.5) ** 2)
        r_norm = r / r.max()  # 最大距離で正規化 (0-1)
        # 氷量 = max * (1 - r_norm) : 中心で max, 縁で 0
        ...
        # パッチ状の氷分布
        ice_map = np.full((ny, nx), config.ice_content_min)  # 初期値 min
        n_patches = 5  # パッチ数
...
def create_alis_mock(config: ALISMockConfig | None = None):
    ...
    # Eq. (1) による校正線勾配の予測
    # 粒径に応じた反射率を内挿 (dry_mineral_spectrum と同じ計算)
    ...
    for iy in range(ny):
        for ix in range(nx):
            ...
            # 検出器ノイズの付加 (SNR ベース)
            if config.snr > 0:
                # ノイズ標準偏差 = 反射率 / SNR (1.5 μm 付近の反射率を使用)
...
def validate_calibration(
    ...
    グラフ解釈:
    - 横軸 (Horizontal axis): 水氷含有量 (wt.%) - 各ピクセルの水氷量を示す。0から最大値まで。
    - 縦軸 (Vertical axis): 1.5 μm バンド深さ (Band Depth) - 水氷吸収の強さを示す指標。値が大きいほど吸収が強い。
    - 青い点 (Mock data): 生成したモックデータの散布図。各点が1ピクセルに対応。氷量が多いほどBDが大きくなる傾向。
    - 赤い実線 (Solid line): Eq. (1) による理論校正線。氷量とBDの理想的な線形関係を示す。
    - 点の分布が線に沿うほど、モデルが正確。外れ値はノイズやモデル誤差を示唆。
    - グリッド線 (Gridlines): 読みやすさのための背景線。
    - 凡例 (Legend): データの種類を説明。
    """
    ...
    ice_range = np.linspace(0, ice_map.max(), 100)  # 0 から max までの範囲
    bd_theory = gradient * ice_range / 100.0  # BD = gradient * ice / 100

    # プロット作成
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))

    # 散布図（全データをプロットすると多すぎるのでサンプリング）
    n_samples = min(2000, len(ice_flat))  # 最大2000点
    ...
    ax.scatter(ice_flat[idx], bd_flat[idx], s=4, alpha=0.3, color="steelblue",
               label="Generated mock data points")

    # 理論校正線
    ax.plot(ice_range, bd_theory, "r-", linewidth=2,
            label=f"Theoretical calibration line (Eq. 1, gradient={gradient:.2f})")  # Solid line (実線): 理論校正線

    # 軸ラベル・タイトル
    ax.set_xlabel("Water ice content (wt.%)", fontsize=12)  # Horizontal axis (横軸): 水氷含有量 (wt.%)
    ax.set_ylabel("1.5-μm Band Depth", fontsize=12)  # Vertical axis (縦軸): 1.5 μm バンド深さ
    ax.set_title(
    ...
    ax.legend(fontsize=10)  # Legend (凡例): データの説明
    ax.set_xlim(left=0)  # Lower limit (下限): x軸の下限を0に設定
    ax.set_ylim(bottom=-0.01)  # Lower limit (下限): y軸の下限を-0.01に設定
    ax.grid(True, alpha=0.3)  # Gridlines (グリッド線): 背景のグリッドを表示
...
def save_montage(
    ...
    グラフ解釈:
    - 上段6枚: 代表波長 (800, 1000, 1200, 1400, 1500, 1600 nm) の反射率画像。各画像は空間分布を示し、色が明るいほど反射率が高い。
    - 左下: 氷量マップ (Ice content) - 青い色が濃いほど水氷含有量が多い。PSRの中心で高く、縁で低い分布を想定。
    - 右下: BDマップ (Band Depth) - 赤い色が濃いほど1.5 μm吸収が強い。水氷量が多い領域でBDが高いはず。
    - カラーバー (Axis label): 各画像の値の範囲を示す。反射率は0-1程度、氷量はwt.%、BDは0-0.1程度。
    - 全体タイトル: 鉱物種、粒径、勾配などのパラメータを要約。
    - このモンタージュで、スペクトルデータと水氷分布の整合性を視覚的に確認できる。
    """
    out_path = Path(out_path)

    # 代表波長の選択 (800, 1000, 1200, 1400, 1500, 1600 nm)
    ...
    # 反射率画像 (6枚)
    vmin_r, vmax_r = np.percentile(cube, [2, 98])  # 2-98% の範囲でクリップ (Data range: データ範囲)
    for i, bidx in enumerate(band_indices):
        wl = wavelengths[bidx]
        im = axes[i].imshow(cube[:, :, bidx], origin="lower", cmap="viridis",
                            vmin=vmin_r, vmax=vmax_r)
        axes[i].set_title(f"Reflectance at {wl:.0f} nm", fontsize=11)
        fig.colorbar(im, ax=axes[i], fraction=0.046, pad=0.04,
                     label="Reflectance")  # Axis label (軸ラベル): カラーバーのラベル

    # 氷量マップ (7番目)
    im_ice = axes[6].imshow(ice_map, origin="lower", cmap="Blues")
    axes[6].set_title("Water ice content map", fontsize=11)
    fig.colorbar(im_ice, ax=axes[6], fraction=0.046, pad=0.04,
                 label="Ice content (wt.%)")  # Axis label (軸ラベル): カラーバーのラベル

    # BD マップ (8番目)
    im_bd = axes[7].imshow(bd_map, origin="lower", cmap="magma")
    axes[7].set_title("1.5-μm Band Depth map", fontsize=11)
    fig.colorbar(im_bd, ax=axes[7], fraction=0.046, pad=0.04,
                 label="Band Depth")  # Axis label (軸ラベル): カラーバーのラベル

    # 全体タイトル
    fig.suptitle(
        f"ALIS Mock Hyperspectral Cube Montage\n"
        f"Mineral: {metadata['mineral_type']} | "
        f"Grain size: {metadata['grain_size_um']:.0f} μm | "
        f"Calibration gradient: {metadata['gradient_predicted_eq1']:.2f}\n"
...
def plot_spectra_comparison(
    ...
    グラフ解釈:
    - 横軸 (Horizontal axis): 波長 (nm) - 750-1650 nm のALIS観測範囲。1.5 μm付近で水氷吸収が現れる。
    - 縦軸 (Vertical axis): 反射率 - 乾燥状態で0.5-0.7程度。水氷吸収で1.5 μm付近が低下。
    - 色付き実線 (Solid line): 異なる氷量のピクセルスペクトル。氷量が多いほど1.5 μm吸収が深くなる。
    - 黒い点線 (Dashed line): 乾燥スペクトル (Dry, no ice) - 水氷なしの基準スペクトル。
    - 凡例 (Legend): 各曲線の氷量と分位数 (q=5%, 25%, 50%, 95%) を示す。分位数が高いほど氷量が多い。
    - グリッド線 (Gridlines): 波長ごとの反射率変化を読みやすくする。
    - このプロットで、水氷量の増加に伴うスペクトル変化を確認。論文Fig.4と比較してモデルの妥当性を検証。
    """
    out_path = Path(out_path)

    # 氷量で4分位のピクセルを選択
    ice_flat = ice_map.ravel()
    quantiles = [0.05, 0.25, 0.50, 0.95]  # 5%, 25%, 50%, 95% 分位
    thresholds = np.quantile(ice_flat, quantiles)  # 各分位の値

    # プロット
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    colors = ["#2166ac", "#4393c3", "#f4a582", "#b2182b"]  # 青系から赤系
    ...
    for q, thresh, color in zip(quantiles, thresholds, colors):
        ...
        ax.plot(wavelengths, spec, color=color, linewidth=1.2,
                label=f"Ice content: {ice_val:.2f} wt.% (quantile {q:.0%})")  # Solid line (実線): スペクトル曲線
    ...
    ax.plot(wavelengths, dry_spec, "k--", linewidth=1.5, label="Dry mineral spectrum (no ice)")  # Dashed line (点線): 乾燥スペクトル

    # 軸設定
    ax.set_xlabel("Wavelength (nm)", fontsize=12)  # Horizontal axis (横軸): 波長 (nm)
    ax.set_ylabel("Reflectance", fontsize=12)  # Vertical axis (縦軸): 反射率
    ax.set_title(
        f"Spectral Comparison for {metadata['mineral_type']}\n"
        f"(Corresponding to Araki & Saiki 2025, Fig. 4)",
        fontsize=12,
    )
    ax.legend(fontsize=9, loc="lower right")  # Legend (凡例): スペクトルの説明
    ax.set_xlim(750, 1650)  # Data range (データ範囲): x軸の範囲を750-1650 nmに設定
    ax.grid(True, alpha=0.3)  # Gridlines (グリッド線): 背景のグリッドを表示
...
if __name__ == "__main__":
    # --- 全鉱物種での生成・検証 ---
    out_dir = Path(__file__).parent / "alis_mock_output"
    out_dir.mkdir(exist_ok=True)  # 出力ディレクトリ作成 (存在しなければ)
